...
* [`src/cljs/chrisblatchley/roast_buddy/`](src/cljs/chrisblatchley/roast_buddy/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/chrisblatchley/roast_buddy/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `roast-buddy` project root directory
...
The `debug?` variable in [`config.cljs`](src/chrisblatchley/roast_buddy/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns chrisblatchley.roast-buddy.example
  (:require [chrisblatchley.roast-buddy.config :as config])
